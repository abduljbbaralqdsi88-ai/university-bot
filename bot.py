...
import pandas as pd
import pytz  # أضف هذه المكتبة للساعات

# ============= الإعدادات =============
TOKEN = os.environ.get("TELEGRAM_BOT_TOKEN")
OWNER_ID = int(os.environ.get("OWNER_ID", "1812586002"))
GROUP_ID = int(os.environ.get("GROUP_ID", "-1003709487288"))
CHANNEL_LINK = "https://t.me/+2GOhgqO8jLVlM2Y0"
DEVELOPER = "Abduljbbar AL_Qdasi"

# التأكد من وجود التوكين
if not TOKEN:
    raise ValueError("❌ TELEGRAM_BOT_TOKEN not found in environment variables!")

# المسارات - استخدام المسار المطلق لضمان العمل على Railway
BASE_DIR = os.path.dirname(os.path.abspath(__file__))
DB_PATH = os.path.join(BASE_DIR, "university_bot.db")
ADMINS_FILE = os.path.join(BASE_DIR, "admins.json")
DAILY_MSG_FILE = os.path.join(BASE_DIR, "daily_msg.json")
HTML = "HTML"

# منطقة زمنية للسعودية
...
def ensure_files():
    ...
    if not os.path.exists(ADMINS_FILE):
        with open(ADMINS_FILE, "w", encoding="utf-8") as f:
            json.dump({}, f)
        print("✅ Created admins.json")
    
    if not os.path.exists(DAILY_MSG_FILE):
        with open(DAILY_MSG_FILE, "w", encoding="utf-8") as f:
            json.dump({"enabled": False, "time": "09:00", "text": ""}, f)
        print("✅ Created daily_msg.json")
...
def has_permission(user_id, perm):
    ...
    return user_adm and user_adm.get(perm, False)
...
def log_sync(user_id, action, details=""):
    ...
    conn.execute(
        ...
        (user_id, action, details, get_now_saudi().isoformat())
    )
    conn.commit()
    conn.close()
...
# دوال قاعدة البيانات (نفس الكود الأصلي - مختصر هنا للتوفير)
def db_departments():
    conn = get_conn()
    rows = conn.execute("SELECT id, emoji, name FROM departments ORDER BY sort_order, id").fetchall()
    conn.close()
    return rows
...
def db_levels(dept_id):
    conn = get_conn()
    rows = conn.execute("SELECT id, name FROM levels WHERE dept_id=? ORDER BY sort_order, id", (dept_id,)).fetchall()
    conn.close()
    return rows
...
def db_subjects(level_id):
    conn = get_conn()
    rows = conn.execute("SELECT id, name FROM subjects WHERE level_id=? ORDER BY sort_order, id", (level_id,)).fetchall()
    conn.close()
    return rows
...
# ====================================================================
#  دوال المفاتيح (نفس الكود الأصلي - مختصر)
# ====================================================================
...
def departments_keyboard(mode="b"):
    ...
    kb = [[InlineKeyboardButton(f"{emoji} {name}", callback_data=f"{mode}D{did}")] for did, emoji, name in depts]
...
def levels_keyboard(dept_id, mode="b"):
    ...
    kb = [[InlineKeyboardButton(f"📚 {name}", callback_data=f"{mode}L{dept_id}_{lid}")] for lid, name in levels]
...
def subjects_keyboard(dept_id, level_id, mode="b"):
    ...
    kb = [[InlineKeyboardButton(f"📖 {name}", callback_data=f"{mode}S{dept_id}_{level_id}_{sid}")] for sid, name in subjs]
    kb.append([
...
def categories_keyboard(dept_id, level_id, subj_id, mode="b"):
    kb = [[InlineKeyboardButton(f"{CAT_EMOJIS[ci]} {cat}", callback_data=f"{mode}C{dept_id}_{level_id}_{subj_id}_{ci}")] for ci, cat in enumerate(CATEGORIES)]
    kb.append([
...
def files_view_keyboard(dept_id, level_id, subj_id, ci):
    kb = [
        [InlineKeyboardButton("📤 إضافة ملف لهذا القسم", callback_data=f"uC{dept_id}_{level_id}_{subj_id}_{ci}")],
...
def adm_files_manage_keyboard(page=0):
    conn = get_conn()
    files = conn.execute("SELECT id, file_name FROM files ORDER BY id DESC LIMIT 10 OFFSET ?", (page * 10,)).fetchall()
    conn.close()
    
    kb = [[InlineKeyboardButton(f"📄 {fname[:25]}", callback_data="none"), InlineKeyboardButton("🗑 حذف", callback_data=f"adm_fdel_{fid}_{page}")] for fid, fname in files]
...
async def addadmin_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    kb = [[InlineKeyboardButton(f"➕ {perm}", callback_data=f"adm_perm_add_{perm}")] for perm in PERMISSIONS]
...
async def addjob_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        await update.message.reply_text("📌 الاستخدام: `/addjob 0 21:00 نص الرسالة`\n\n• اليوم: 0=الاثنين إلى 6=الأحد", parse_mode=HTML)
        return
    ...
    if not match:
        await update.message.reply_text("❌ صيغة غير صحيحة.\nالصيغة: `/addjob 0 21:00 نص الرسالة`", parse_mode=HTML)
        return
    
    day, time_str, text = int(match.group(1)), match.group(2), match.group(3)
    ...
    try:
        ...
        job = context.job_queue.run_daily(
            send_job_message,
            time=time(hour, minute, tzinfo=SAUDI_TZ),
            days=(days_map[day],),
            name=f"weekly_{day}_{time_str}_{int(get_now_saudi().timestamp())}"
        )
        
        if "jobs_list" not in context.bot_data:
            context.bot_data["jobs_list"] = []
        context.bot_data["jobs_list"].append({
            "type": "weekly", "day": day, "time": time_str, "text": text, "job_name": job.name
        })
        
        days_ar = ["الاثنين", "الثلاثاء", "الأربعاء", "الخميس", "الجمعة", "السبت", "الأحد"]
        await update.message.reply_text(f"✅ تم إضافة الموعد الأسبوعي!\n📅 اليوم: {days_ar[day]}\n⏰ الوقت: {time_str}", parse_mode=HTML)
    except Exception as e:
...
async def oncejob_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        await update.message.reply_text("📌 الاستخدام: `/oncejob 2026-10-20 21:00 نص الرسالة`", parse_mode=HTML)
        return
    ...
    if not match:
        await update.message.reply_text("❌ صيغة غير صحيحة.", parse_mode=HTML)
        return
    ...
    try:
        ...
        target_time = SAUDI_TZ.localize(target_time)
        
        if target_time < get_now_saudi():
        ...
        job = context.job_queue.run_once(send_once_message, when=target_time, name=f"once_{date_str}_{time_str}_{int(get_now_saudi().timestamp())}")
        
        if "jobs_list" not in context.bot_data:
            context.bot_data["jobs_list"] = []
        context.bot_data["jobs_list"].append({
            "type": "once", "date": date_str, "time": time_str, "text": text, "job_name": job.name
        })
        
        await update.message.reply_text(f"✅ تم إضافة الموعد (مرة واحدة)!\n📅 التاريخ: {date_str}\n⏰ الوقت: {time_str}", parse_mode=HTML)
    except Exception as e:
        await update.message.reply_text(f"❌ حدث خطأ: {e}")
...
async def setdaily_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        await update.message.reply_text("📌 الاستخدام: `/setdaily 09:00 نص الرسالة`", parse_mode=HTML)
        return
    ...
    daily_data = {"enabled": True, "time": time_str, "text": text}
    save_daily_msg(daily_data)
    
    # إعادة تشغيل المهمة اليومية
    await setup_daily_job(context.application)
    
    await update.message.reply_text(f"✅ تم تحديث الرسالة اليومية!\n⏰ الوقت: {time_str}", parse_mode=HTML)
...
async def deljob_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        await update.message.reply_text("📌 الاستخدام: `/deljob job_name`\nاستخدم /listjobs لعرض الأسماء", parse_mode=HTML)
        return
    ...
    await update.message.reply_text(f"✅ تم حذف الموعد: `{job_name}`" if removed else f"❌ لم يتم العثور على الموعد", parse_mode=HTML)
...
async def delday_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        await update.message.reply_text("📌 الاستخدام: `/delday 0`\n0=الاثنين إلى 6=الأحد", parse_mode=HTML)
        return
    ...
    days_ar = ["الاثنين", "الثلاثاء", "الأربعاء", "الخميس", "الجمعة", "السبت", "الأحد"]
    await update.message.reply_text(f"✅ تم حذف {removed_count} موعد/مواعيد ليوم {days_ar[day_num]}.")
...
async def listjobs_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for job_info in context.bot_data["jobs_list"]:
        if job_info["type"] == "weekly":
            lines.append(f"🔄 أسبوعي | {days_ar[job_info['day']]} | {job_info['time']}\n   📝 {job_info['text'][:50]}...\n   🆔 `{job_info['job_name']}`\n")
        else:
            lines.append(f"⏰ لمرة واحدة | {job_info['date']} {job_info['time']}\n   📝 {job_info['text'][:50]}...\n   🆔 `{job_info['job_name']}`\n")
...
async def send_daily_message(context: ContextTypes.DEFAULT_TYPE):
    ...
    if daily.get("enabled") and daily.get("text"):
        try:
            ...
            print(f"✅ Daily message sent at {get_now_saudi()}")
        except Exception as e:
...
async def setup_daily_job(application: Application):
    ...
    if daily.get("enabled") and daily.get("time"):
        try:
            ...
            print(f"✅ Daily job scheduled at {time_str} Riyadh time")
        except Exception as e:
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("adm_perm_"):
        ...
        if data == "adm_perm_done":
            ...
            save_admins(admins)
            context.user_data.pop("pending_admin", None)
            await query.edit_message_text(f"✅ تم إضافة المستخدم <code>{pending}</code> كأدمن!", parse_mode=HTML)
            return
    ...
    if data == "main":
        ...
        await query.edit_message_text("🎓 <b>القائمة الرئيسية</b>", reply_markup=main_keyboard(user.id), parse_mode=HTML)
        return

    if data == "browse":
        context.user_data.clear()
        await query.edit_message_text("🏛 <b>اختر القسم:</b>", reply_markup=departments_keyboard("b"), parse_mode=HTML)
        return

    if data == "upload":
        context.user_data.clear()
        await query.edit_message_text("📤 <b>رفع ملف — اختر القسم:</b>", reply_markup=departments_keyboard("u"), parse_mode=HTML)
        return
    ...
    if data.startswith("bD") or data.startswith("uD"):
        ...
        if not dept:
            ...
            return
        _, emoji, name = dept
        await query.edit_message_text(f"{emoji} <b>{h(name)}</b>\n\nاختر المستوى:", reply_markup=levels_keyboard(dept_id, mode), parse_mode=HTML)
        return
    ...
    if (data.startswith("bL") or data.startswith("uL")) and data.count("_") == 1:
        ...
        _, emoji, dname = dept
        _, lname, _ = level
        await query.edit_message_text(f"{emoji} <b>{h(dname)} | {h(lname)}</b>\n\nاختر المادة:", reply_markup=subjects_keyboard(dept_id, level_id, mode), parse_mode=HTML)
        return
    ...
    if (data.startswith("bS") or data.startswith("uS")) and "_" in data:
        ...
        if not subj:
            ...
            return
        _, sname, _ = subj
        await query.edit_message_text(f"📖 <b>{h(sname)}</b>\n\nاختر نوع الملف:", reply_markup=categories_keyboard(dept_id, level_id, subj_id, mode), parse_mode=HTML)
        return
    ...
    if data.startswith("bC") and "_" in data:
        ...
        conn = get_conn()
        files = conn.execute("SELECT file_name, telegram_file_id FROM files WHERE dept_id=? AND level_id=? AND subject_id=? AND cat_idx=?", (dept_id, level_id, subj_id, ci)).fetchall()
        conn.close()

        if files:
            await query.edit_message_text(f"📂 <b>{h(sname)} | {h(cat)}</b>\n\n✅ يوجد <b>{len(files)}</b> ملف، جاري الإرسال...", reply_markup=files_view_keyboard(dept_id, level_id, subj_id, ci), parse_mode=HTML)
            for fname, fid in files:
                try:
                    await context.bot.send_document(chat_id=user.id, document=fid, caption=f"📄 {fname}\n📂 {sname} | {cat}")
                except Exception:
                    await query.message.reply_text(f"⚠️ تعذّر إرسال: {fname}")
        else:
            await query.edit_message_text(f"📂 <b>{h(sname)} | {h(cat)}</b>\n\n❌ لا توجد ملفات بعد.\n\nكن أول من يساهم! 👇", reply_markup=files_view_keyboard(dept_id, level_id, subj_id, ci), parse_mode=HTML)
        return
    ...
    if data.startswith("bL") and data.count("_") == 2:
        ...
        await query.edit_message_text(f"{emoji} <b>{h(dname)} | {h(lname)}</b>\n\nاختر المادة:", reply_markup=subjects_keyboard(dept_id, level_id, "b"), parse_mode=HTML)
        return
    ...
    if data.startswith("uC") and "_" in data:
        ...
        context.user_data.update({"awaiting_file": True, "dept_id": dept_id, "level_id": level_id, "subj_id": subj_id, "ci": ci})
        await query.edit_message_text(f"📤 <b>رفع ملف</b>\n\n📂 {h(dname)} ‹ {h(lname)} ‹ {h(sname)} ‹ {h(cat)}\n\n✅ أرسل الملف الآن:", parse_mode=HTML)
        return
    ...
    if data == "adm_manage_files":
        if not has_permission(user.id, "delfile"):
            ...
            return
        await query.edit_message_text("📁 **إدارة الملفات:**\nاختر الملف الذي تريد حذفه:", reply_markup=adm_files_manage_keyboard(0), parse_mode=HTML)
        return
    ...
    if data.startswith("adm_fpage_"):
        ...
        await query.edit_message_text("📁 **إدارة الملفات:**", reply_markup=adm_files_manage_keyboard(page), parse_mode=HTML)
        return
    ...
    if data == "adm_depts":
        if not has_permission(user.id, "admin"):
            ...
            return
        await query.edit_message_text("⚙️ <b>إدارة الأقسام</b>", reply_markup=adm_depts_keyboard(), parse_mode=HTML)
        return

    # باقي معالجات الأدمن (مختصرة للاختصار - نفس الكود الأصلي)
...
async def _admin_main(query):
    ...
    text = f"👑 <b>لوحة تحكم الأدمن</b>\n━━━━━━━━━━━━━━━━━━━━\n\n👥 المستخدمون: <code>{total_users}</code>\n📁 الملفات: <code>{total_files}</code>\n🏛 الأقسام: <code>{total_depts}</code>"
    await query.edit_message_text(text, reply_markup=admin_main_keyboard(), parse_mode=HTML)


async def _adm_users(query):
    conn = get_conn()
    rows = conn.execute("SELECT user_id, first_name, username, join_date FROM users ORDER BY join_date DESC LIMIT 15").fetchall()
    conn.close()
    lines = ["👥 <b>آخر 15 مستخدم:</b>\n"]
    for uid, fname, uname, jd in rows:
        lines.append(f"• <b>{h(fname or '')}</b> (@{h(uname or '—')})\n  🆔 <code>{uid}</code> | انضم: {(jd or '')[:10]}")
...
async def _adm_logs(query):
    conn = get_conn()
    rows = conn.execute("SELECT l.user_id, u.first_name, l.action, l.timestamp FROM logs l LEFT JOIN users u ON l.user_id=u.user_id ORDER BY l.timestamp DESC LIMIT 20").fetchall()
    conn.close()
    labels = {"start": "▶️ بدأ", "browse": "🔍 تصفّح", "upload_file": "📤 رفع"}
...
async def _adm_excel(query, context, user_id):
    ...
    files_df = pd.read_sql_query("SELECT f.*, d.name dept_name, s.name subject_name FROM files f LEFT JOIN departments d ON f.dept_id=d.id LEFT JOIN subjects s ON f.subject_id=s.id", conn)
    conn.close()
    ...
    await context.bot.send_document(chat_id=user_id, document=output, filename=f"library_{get_now_saudi().strftime('%Y%m%d_%H%M%S')}.xlsx", caption=f"📊 تقرير المكتبة\n👨‍💻 {DEVELOPER}")
...
async def message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action and (user.id == OWNER_ID or has_permission(user.id, "admin")):
        ...
        conn = get_conn()
        if action == "add_dept":
            conn.execute("INSERT INTO departments (emoji,name,sort_order) VALUES ('🏛',?,999)", (text,))
            conn.commit()
            await update.message.reply_text(f"✅ تمت إضافة قسم: <b>{h(text)}</b>", parse_mode=HTML)

        elif action == "edit_dept":
            conn.execute("UPDATE departments SET name=? WHERE id=?", (text, ud["admin_target"]))
            conn.commit()
            await update.message.reply_text(f"✅ تم تعديل اسم القسم إلى: <b>{h(text)}</b>", parse_mode=HTML)

        elif action == "edit_dept_emoji":
            conn.execute("UPDATE departments SET emoji=? WHERE id=?", (text, ud["admin_target"]))
            conn.commit()
            await update.message.reply_text(f"✅ تم تغيير الإيموجي إلى: {h(text)}", parse_mode=HTML)
            ...
            conn.commit()
            await update.message.reply_text(f"✅ تمت إضافة مستوى: <b>{h(text)}</b>", parse_mode=HTML)

        elif action == "edit_level":
            conn.execute("UPDATE levels SET name=? WHERE id=?", (text, ud["admin_target"]))
            conn.commit()
            await update.message.reply_text(f"✅ تم تعديل اسم المستوى إلى: <b>{h(text)}</b>", parse_mode=HTML)

        elif action == "add_subject":
            level_id = ud["admin_target"]
            conn.execute("INSERT INTO subjects (level_id,name,sort_order) VALUES (?,?,999)", (level_id, text))
            conn.commit()
            await update.message.reply_text(f"✅ تمت إضافة مادة: <b>{h(text)}</b>", parse_mode=HTML)

        elif action == "edit_subject":
            conn.execute("UPDATE subjects SET name=? WHERE id=?", (text, ud["admin_target"]))
            conn.commit()
            await update.message.reply_text(f"✅ تم تعديل اسم المادة إلى: <b>{h(text)}</b>", parse_mode=HTML)
    ...
    if ud.get("awaiting_file"):
        ...
        file_name = doc.file_name or f"file_{get_now_saudi().strftime('%Y%m%d_%H%M%S')}"
        file_id = doc.file_id
        file_size = doc.file_size or 0

        dept_id, level_id, subj_id, ci = ud["dept_id"], ud["level_id"], ud["subj_id"], ud["ci"]

        conn = get_conn()
        conn.execute("INSERT INTO files (dept_id,level_id,subject_id,cat_idx,file_name,telegram_file_id,file_size,uploaded_by,upload_date) VALUES (?,?,?,?,?,?,?,?,?)", (dept_id, level_id, subj_id, ci, file_name, file_id, file_size, user.id, get_now_saudi().isoformat()))
        conn.commit()
        conn.close()
        ...
        await update.message.reply_text(f"✅ <b>تم رفع الملف بنجاح!</b>\n\n📄 <code>{h(file_name)}</code>\n📏 {round(file_size/1024,1)} KB\n\n🙏 شكراً على مساهمتك!", parse_mode=HTML, reply_markup=main_keyboard(user.id))
        ud.clear()
        return

    await update.message.reply_text("اضغط /start للعودة للقائمة الرئيسية.")
...
def main():
    ...
    application.add_handler(CommandHandler("addjob", addjob_command))
    application.add_handler(CommandHandler("oncejob", oncejob_command))
    application.add_handler(CommandHandler("setdaily", setdaily_command))
    application.add_handler(CommandHandler("deljob", deljob_command))
    application.add_handler(CommandHandler("delday", delday_command))
    application.add_handler(CommandHandler("listjobs", listjobs_command))
    application.add_handler(CallbackQueryHandler(button_handler))
    application.add_handler(MessageHandler(filters.ALL & ~filters.COMMAND, message_handler))
    application.add_error_handler(error_handler)

    # إعداد المهمة اليومية
    import asyncio
    asyncio.run(setup_daily_job(application))
